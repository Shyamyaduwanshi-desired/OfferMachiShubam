...
public class ExclusiveOffersPresenter {
    private Context context;
    private ExclusiveList exclusiveList;
    private ProgressDialog progress;

    public ExclusiveOffersPresenter(Context context, ExclusiveList exclusiveList) {
        this.context = context;
        this.exclusiveList = exclusiveList;

    }

    public interface ExclusiveList {
    ...
    public void ViewAllExclusive(final String userid) {
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "customer_coupons_by_notifications_data", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    exclusiveList.fail("Something went wrong. Please try after some time.");
                }
            }
            ...
            public void onErrorResponse(VolleyError error) {
                hidepDialog();
                exclusiveList.fail("Server Error.\n Please try after some time.");
            }
        }
        ) {
            @Override
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "customer_add_favourite_offer", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    int status = reader.getInt("status");
                    if(status == 200){
                        exclusiveList.favsuc(reader.getString("message"));

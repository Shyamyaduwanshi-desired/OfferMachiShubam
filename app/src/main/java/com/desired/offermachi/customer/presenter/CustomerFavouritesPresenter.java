...
public class CustomerFavouritesPresenter {
    private Context context;
    private FavouritesList favouritesList;
    private ProgressDialog progress;

    public CustomerFavouritesPresenter(Context context, FavouritesList favouritesList) {
        this.context = context;
        this.favouritesList = favouritesList;

    }

    public interface FavouritesList {
    ...
    public void ViewAllFavourties(final String userid) {
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "select_customer_all_favourite_offers_data", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (status == 200) {
                        ...
                        favouritesList.success(list);


                    } else if (status == 404) {
                        favouritesList.error(reader.getString("message"));
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                    favouritesList.fail("Something went wrong. Please try after some time.");
                }
            }
            ...
            public void onErrorResponse(VolleyError error) {
                hidepDialog();
                favouritesList.fail("Server Error.\n Please try after some time.");
            }
        }
        ) {
            @Override
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "customer_add_favourite_offer", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    int status = reader.getInt("status");
                    if(status == 200){
                        favouritesList.favsuccess(reader.getString("message"));

...
public class CustomerSelectCategoryPresenter {
    private Context context;
    private CategoryList categoryList;
    private ProgressDialog progress;

    public CustomerSelectCategoryPresenter(Context context, CategoryList categoryList) {
    ...
    public void ViewAllCategory(final String catid,final String userid) {
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "customer_single_category_offer_data", new Response.Listener<String>() {
            @Override
              ...
              hidepDialog();
                try {
                    ...
                    if (status == 200) {
                        ...
                        categoryList.error(reader.getString("message"));
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                    categoryList.fail("Something went wrong. Please try after some time.");
                }
            }
            ...
            public void onErrorResponse(VolleyError error) {
               hidepDialog();
                categoryList.fail("Server Error.\n Please try after some time.");
            }
        }
        ) {
            @Override
        ...
        StringRequest postRequest = new StringRequest(Request.Method.POST, AppData.url + "customer_add_favourite_offer", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    int status = reader.getInt("status");
                    if(status == 200){
                        categoryList.favsuccess(reader.getString("message"));
